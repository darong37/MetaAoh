...
入力はソート済みであることを前提とする。各階層の「外側からの累積キー組」（`level1` の組、`level1+level2` の組、…）について、同一のキー組は連続して現れなければならない。一度終わったキー組が離れて再出現した場合は、呼び出し側のソート漏れ（SQL の `ORDER BY` 忘れ等）とみなして `croak` する。ソートされていない入力をグループ化したい場合は、先に `sort()` を呼ぶ。連続した入力に対する結果は従来と変わらないため、`group` / `expand` の可逆性は維持される。
